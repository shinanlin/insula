...
def main(
    ...
    t0 = _time.time()
    logger.info("=== Cross-ROI resolved decoding ===")
    ...
    logger.info(f"  Running resolved decoding: window={window}s, step={step}s, "
    ...
    logger.info(f"  Significant time points (FDR<0.05): {(pvals_fdr < 0.05).sum()} / {pvals_fdr.size}")

    time = np.arange(tmin + window, tmax + step, step)

    save_path = BIDSPath(
        root=f"./results/LexicalDelay(cross_roi)({ref})",
        datatype=f"(cross)(resolved){datatype}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Cross-ROI resolved decoding with CCA alignment"
    )
