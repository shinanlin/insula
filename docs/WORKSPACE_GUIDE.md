...
- `results/`: generated analysis outputs. Prefer method-first roots (`hga/`, `nmf/`, `connectivity/`); see `CODE_STYLE.md`.
...
|------------|------|-----------|
| `src/hga/` | BIDS epoch + parcellation → `results/hga/{task}/` | `src/hga/package_highgamma.py` |
...
Packaging scripts prepare BIDS-derived HGA CSVs under `results/`. Electrode
membership is the per-subject × modality union of significant channels across
all conditions and phases (`epoch(band)(sig)`); waveforms come from
`epoch(band)(zscore)`. Use the `mask` column for within-phase significance.
...
Generated results live under `results/`. **New pipelines are method-first:**
`results/{method}/{Task}/...`. I/O rules (read with `BIDSPath`, write by method)
are in [`CODE_STYLE.md`](CODE_STYLE.md#two-io-rules-required-for-new-code).

Examples:

- `results/hga/{Task}/sub-*/HGA/*_time.csv` — packaged Hammers HGA
- `results/connectivity/{Task}/sub-*/{metric}/` — connectivity derivatives
- `results/decoding/{Task}/sub-*/(decode)(pattern){feature}/` — INS Haufe decoding patterns
- `results/nmf/channel_assignments.csv` — **canonical** functional clusters (concat crop k=3); use `src.paths.nmf_assignments_path()`
- `results/nmf/concat_phases/crop_postonset/k3/` — full NMF run artifacts (downstream should not hard-code this)
- `results/qc/hammers/sub-D0094/` — Stage 3 parcellation slice QC (`index.csv`, `png/`, subject PDF)

Legacy decoding trees (do not extend for new analyses):

- `results/LexicalDelay(roi)(bipolar)`
- `results/PhonemeSequence(roi)(bipolar)`
- `results/LexicalDelay(cross_roi)(bipolar)` (if present)
