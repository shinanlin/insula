#!/bin/bash
#SBATCH --job-name=prep_decode_ins
#SBATCH --output=/hpc/group/coganlab/nanlinshi/insula-functional/logs/slurm/prepare_decoding_insula_%A_%a.out
#SBATCH --error=/hpc/group/coganlab/nanlinshi/insula-functional/logs/slurm/prepare_decoding_insula_%A_%a.err
...
python -u -m src.decoding.prepare_insula_decoding_dataset \
  --bids-root "${BIDS_ROOT}" \
  --bids-task "${BIDS_TASK}" \
