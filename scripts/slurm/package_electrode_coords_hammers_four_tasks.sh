#!/bin/bash
#SBATCH --job-name=pkg_coord_ham4
#SBATCH --output=/hpc/group/coganlab/nanlinshi/insula/logs/slurm/package_electrode_coords_hammers_four_%j.out
#SBATCH --error=/hpc/group/coganlab/nanlinshi/insula/logs/slurm/package_electrode_coords_hammers_four_%j.err
...
run_one() {
  local bids_root="$1"
  local name="$2"
  echo "===== ${name} electrode coords (hammers) ====="
  echo "bids_root=${bids_root} start=$(date -u)"
  python -m src.hga.package_ave_cord \
    --bids_root "${bids_root}" \
    --band highgamma \
    --ref bipolar \
    --atlas hammers
...
echo "All four tasks (hammers electrode coords) done."
