#!/bin/bash
# Windowed decoding for left-hemisphere pseudo-subjects only.
# Array index maps to: subject × description × datatype × phase (band fixed).
# Phase windows are hardcoded in src/decoding/run_decoding.py (PHASE_WINDOWS).
#
# Invoked by scripts/slurm/decoding_*_left.sh (preferred) or locally:
#   TASK=LexicalDelay bash scripts/decoding_left.sh
