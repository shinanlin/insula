...
import { PHASE_TO_EXPORT_KEY, PHASE_TIME_RANGES } from '../constants/waveform.js';
import { windowMean } from './traceInterp.js';
...
function resolveElectrodePhaseTrace(tracesBySubject, electrode, task, phase) {
  const exportPhase = PHASE_TO_EXPORT_KEY[phase];
  if (!exportPhase) return null;
  return tracesBySubject[electrode.subject]?.[electrode.id]?.[task]?.[exportPhase] ?? null;
}

function causalWindowMeanForElectrode(tracesBySubject, electrode, task, phase, time, windowSec) {
  const trace = resolveElectrodePhaseTrace(tracesBySubject, electrode, task, phase);
...
export function buildSlidingWindowFrames(
  ...
  phase,
  selectedTask,
  { windowSec = ANIM_WINDOW_SEC, stepSec = ANIM_STEP_SEC } = {},
) {
...
export function bundleHasPlayableFrames(bundle) {
  return Boolean(bundle?.frames?.length);
}
