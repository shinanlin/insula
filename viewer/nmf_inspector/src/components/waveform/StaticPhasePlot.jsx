...
import { PHASE_LABELS, PHASE_TIME_RANGES } from '../../constants/waveform.js';
import {
...
import { buildWaveformPlotData } from '../../utils/waveformTraces.js';
...
export default function StaticPhasePlot({
  phase,
  index,
  trace,
  traceKey,
  ...
  const layout = useMemo(() => ({
    ...
    xaxis: {
      title: {
        text: 'Time (s)',
        font: { family: PLOT_FONT_FAMILY, size: PLOT_AXIS_TITLE_SIZE, color: '#334155' },
        standoff: 6,
      },
      range: [min, max],
      autorange: false,
  ...
  if (!trace) {
    return <div className="plot-empty">No HGA trace for {PHASE_LABELS[phase]}</div>;
  }
